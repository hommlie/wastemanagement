// adminFrontend/src/pages/ActionsPage.jsx
...
export default function ActionsPage() {
  ...
  function openAdd() {
    if (!can("actions", "create")) {
      toast.error("You do not have permission to create actions");
      return;
    }
    setEditingId(null);
    setForm({ code: "", title: "" });
  ...
  function openEdit(a) {
    if (!can("actions", "edit")) {
      toast.error("You do not have permission to edit actions");
      return;
    }
    setEditingId(a.id);
  ...
  async function handleSave(e) {
    ...
    if (editingId && !can("actions", "edit")) {
      toast.error("You do not have permission to update actions");
      return;
    }
    if (!editingId && !can("actions", "create")) {
      toast.error("You do not have permission to create actions");
      return;
    }
    ...
    try {
      ...
      let res, json;
      if (editingId) {
        res = await api.put(`/actions/${editingId}`, payload);
      } else {
        res = await api.post(`/actions`, payload);
      }
      json = await res.json();
      const appError =
  ...
  async function doDelete(a) {
    if (!can("actions", "delete")) {
      toast.error("You do not have permission to delete actions");
      return;
    }
    try {
      const res = await api.del(`/actions/${a.id}`);
      const json = await res.json();
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Create, edit and manage actions (create, view, edit, delete, export).
          </p>
        </div>
          ...
          >
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder="create / view / edit / delete"
              />
            </div>
      ...
      <ConfirmModal
        ...
        message={
          confirmAction?.action
            ? `Delete action "${confirmAction.action.title}"? This cannot be undone.`
            : "Are you sure?"
        }
        confirmText="Delete"
        cancelText="Cancel"
