...
export default function StatesPage() {
  // ---------- PERMISSION GUARD ----------
  // yaha ka slug "states" / "state" ko apne permission ke hisaab se same rakho
  if (!can("state", "view")) {
    return (
  ...
  const fetchStates = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        // api wrapper use karo, ye hi headers/cookies add karta hai
        const res = await api.get(`/state?${params.toString()}`);
        ...
        if (res.ok && (json.status === 1 || json.status === undefined)) {
          ...
          // backend Unauthorized de to toast mat dikhao
