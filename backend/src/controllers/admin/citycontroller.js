...
exports.getState = async (req, res) => {
  try {
    const states = await State.findAll({ order: [["name", "ASC"]], attributes: ["id", "name", "code", "status", "created_at", "updated_at"] });
    return res.status(200).json({ status: 1, message: "States fetched successfully", data: states });
  }
  catch (error) {
    return res.status(500).json({ status: 0, message: error.message });
  }
